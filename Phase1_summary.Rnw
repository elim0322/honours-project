...
library(knitr)
knit("example1.Rhtml")
# system("wkhtmltopdf http://... ~/.../example1.pdf")
@
...
Figure \ref{fig:ex1} shows what ``example1.html" will look like in a browser.
\begin{figure}[b]
\centering
...
\begin{figure}[h!]
  \begin{tikzpicture}[node distance=2cm]
    ...
    \node(browser)[simple, right of=snap, xshift=2cm]{browser: upload, edit and save};
    ...
    \draw[arrow] (snap) to [bend left = 40] (browser);
    \draw[arrow] (browser) to [bend left = 40] (snap);
    %
...
Ideally the reviewer edits directly on the post.html document prepared by the document provider and saves the changes. Then the document provider can re-generate the source document with appropriate changes which can be knitted again to produce another post.html document for editing.

Much detail is omitted for the purpose of describing the overview. Important assumptions and more detailed discussion of these functions involved in the round trip will be.
