...
% Throughout the project, many important assumptions have emerged from time to time which have played an important role in improving the robustness of our methodology on the completion of the round trip. These assumptions should be regarded as .... Therefore, it is absolutely necessary to mention them along with more detailed discussion on the reasoning behind the logic of our functions.

% Function:sew()
% We decided to use the HTML comment method so that the copies are not displayed (because they are comments) and other benefits. The sections of any R related contents in final documents should not be directly modified considering the unidirectional property of \textbf{knitr} (explored in Section \ref{sec:introduction})...

% snap()
% The reason for uploading onto the test server is so the user does not have to go through localizing CKEditor in his/her device. i.e. CKEditor is already enabled.
% Reason for choosing JavaScript and CKEditor & AnnotateIt.
% A possible restriction with using \textbf{AnnotateIt} may be its requirement for users to be logged in on their webiste (\url{http://annotateit.org}). Despite this requirement, it has been tested to be fairly reliable and meet satisfactory standards of our purpose.
% Use of XML for robust/precise searching.
% There are two supplementary sections of code that are defined externally in separate files so that we can explicitly make changes on them without changing snap() itself.

% annotations()
% The text file is in the JSON format.
% Complications from annotating on the same words..
